...
int main()
{
  ...
  std::string input_to_PostfixEvaluator = InfixToPostfix(input);

  int output = PostfixEvaluator(input_to_PostfixEvaluator);
 ...
 *conceived after analysing the example from the following
 *link:
...
std::string InfixToPostfix(std::string input)
{
  ...
  /*input_to_PostfixEvaluator has been initialized with it's
    size being the same as the size of the input. This is
 ...
 *precedence of '-'. And it the case with '*' and '/' also.
 *So any number can be used to quantify the precedence of
 *these operators, given that the above rules are satisfied.
 ************************************************************/
